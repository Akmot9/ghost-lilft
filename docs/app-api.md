...
| `set_exercise_bodyweight` | `seanceSlug`, `exerciseSlug`, `isBodyweight` | `Exercise` mis à jour ; ses séries peuvent désormais porter un lest nul. Le retirer est refusé tant que des séries sans lest restent (elles ne seraient plus ni lisibles ni corrigibles) | `charge-invalide`, `introuvable`, `stockage-indisponible` |
...
| `update_exercise` | `seanceSlug`, `exerciseSlug`, `input: CreateExerciseInput` | `Seance` canonique ; le **slug ne bouge pas** — c'est l'identité dont dépendent le routage, les fantômes et l'historique. Passer `isBodyweight` à faux est refusé tant que des séries sans lest restent | codes de validation, `introuvable`, `stockage-indisponible` |
