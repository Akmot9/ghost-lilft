...
function weeklyTotals(sets: ExerciseSet[]): number[] {
  return weeklyVolumes(sets).map((week) => week.volume)
}
...
describe('programme de démonstration', () => {
  ...
  it('couvre au moins huit semaines', () => {
    expect(weeklyVolumes(program.flatMap(allSetsOf)).length).toBeGreaterThanOrEqual(8)
  })
  ...
  it('Upper A raconte une progression franche', () => {
    const volumes = weeklyTotals(allSetsOf(seance('upper-a')))
  ...
  it('Upper B traverse une semaine en baisse, puis repart', () => {
    const volumes = weeklyTotals(allSetsOf(seance('upper-b')))
