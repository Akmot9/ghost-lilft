...
const INSERT_EXERCISE_SQL =
  'INSERT INTO exercises (seance_slug, slug, name, default_reps, default_weight, weight_unit, rest_seconds, position) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)'
...
async function loadSeancesFromDatabase(database: Database): Promise<Seance[]> {
  const [seanceRows, exerciseRows, setRows] = await Promise.all([
    ...
    database.select<ExerciseRow[]>(
      'SELECT seance_slug, slug, name, default_reps, default_weight, weight_unit, rest_seconds FROM exercises ORDER BY position, rowid',
    ),
